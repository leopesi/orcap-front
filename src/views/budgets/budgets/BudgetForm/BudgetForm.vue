<template>
  <div class="home">
          ...
          <div class="form-group mb-3">
            ...
            <div class="input-group mb-3">
              ...
              >
                ...
                <option
                  :value="seller.id"
                  v-for="(seller, i) in this.sellers"
                  :key="i"
                >
                  {{ seller.name }}
                </option>
          ...
          <Card class="card" v-if="this.form">
            ...
            <div class="">
              <div class="row" v-if="this.form">
                <div
                  class="col-sm-4 pb-4"
                  v-for="(equipment, i) in this.form.equipments"
                  :key="i"
                >
                  <Filters
          ...
          <div class="card" v-if="this.form">
            ...
            <div class="">
              <div class="row">
                  ...
                  <div class="form-group mb-3">
                    ...
                    <div class="input-group mb-3">
                      ...
                      >
                        ...
                        <option
                          :value="i"
                          v-for="(payment, i) in this.payments"
                          :key="i"
                        >
                          {{ payment }}
                        </option>
                ...
                <div class="col-sm-2">
                  <div class="form-group">
                    <label for="installment_tax">
                      {{ $t('installment_tax') }}
                    </label>
                    <input
                      class="form-control"
                      id="installment_tax"
                      v-model="form.installment_tax"
                      type="number"
                      disabled
                    />
                  </div>
                ...
                <div class="col-sm-2">
                  <div class="form-group">
                    ...
                    <input
                      ...
                      :value="
                        parseFloat(
                          form.forward_price_total / form.installment_number,
                        ).toFixed(2)
                      "
                      type="number"
                      disabled
                ...
                <div class="col-sm-2">
                  <div class="form-group">
                    ...
                    <input
                      ...
                      id="cash_price_total"
                      type="text"
                      :value="this.form.cash_price_total"
                      disabled
                    />
                  </div>
                </div>
                <div class="col-sm-2">
                  <div class="form-group">
                    <label for="forward_price_total">
                      {{ $t('forward_price_total') }}
                    </label>
                    <input
                      class="form-control"
                      id="forward_price_total"
                      type="text"
                      :value="this.form.forward_price_total"
                      disabled
                    />
                  </div>
                </div>
    ...
    <FloatCard
      ...
      :show="this.showAddEquipment"
      @close="showAddEquipment = false"
    >
      <div class="row">
        <div class="col-sm-12">
          <div class="form-group mb-3">
            ...
            <div class="input-group mb-3">
              ...
              >
                ...
                <option
                  :value="i"
                  v-for="(equipment, i) in this.equipments"
                  :key="i"
                >
                  {{ equipment }}
                </option>
...
export default {
  ...
  methods: {
    load() {
      ...
      if (this.id) {
        Budgets.getBudget(this.id, (result) => {
          ...
          if (result.data) {
            this.form.expiration_date = Methods.fixSequelizeOnlyDate(
    ...
    save() {
      if (this.id && this.id != 0) {
        ...
        Budgets.insertBudget(this.form, (result) => {
          ...
          this.form.expiration_date = Methods.fixSequelizeDate(
    ...
    changedValues() {
      ...
      const construction_labor =
        parseFloat(this.form.construction_labor) * this.form.m2_total
      const excavation_labor =
        parseFloat(this.form.excavation_labor) * this.form.m3_total
      const earth_removal_labor =
        parseFloat(this.form.earth_removal_labor) * this.form.m3_total * 1.2
      const short_wall_labor = parseFloat(this.form.short_wall_labor)
      const subfloor_labor =
        parseFloat(this.form.subfloor_labor) *
        (this.form.perimeter * this.form.sidewalk_width +
          this.form.sidewalk_width * this.form.sidewalk_width * 4)
      const material_placement_labor = parseFloat(
        this.form.material_placement_labor,
      )
      ...
      this.form.cash_price += isNaN(earth_removal_labor)
        ? 0
        : earth_removal_labor
      this.form.cash_price += isNaN(short_wall_labor) ? 0 : short_wall_labor
      this.form.cash_price += isNaN(subfloor_labor) ? 0 : subfloor_labor
      this.form.cash_price += isNaN(material_placement_labor)
        ? 0
        : material_placement_labor
      this.form.cash_price += isNaN(reserve) ? 0 : reserve
      this.form.cash_price += isNaN(job_monitoring_fee) ? 0 : job_monitoring_fee

      this.form.cash_price_total = parseFloat(
        this.form.cash_price -
          (isNaN(this.form.discount) ? 0 : this.form.discount) +
          (this.form.cash_price * (isNaN(this.form.art) ? 0 : this.form.art)) /
            100,
      )

      this.form.cash_price = isNaN(this.form.cash_price)
        ? 0
        : parseFloat(this.form.cash_price).toFixed(2)
      this.form.cash_price_total = isNaN(this.form.cash_price_total)
        ? 0
        : parseFloat(this.form.cash_price_total).toFixed(2)
    },
    changeTax() {
      try {
        this.changedValues()
        if (this.form.installment_number > 0) {
          if (this.form.payment == 'in_cash') {
            ...
            this.form.forward_price = parseFloat(this.form.cash_price).toFixed(
              2,
            )
            this.form.forward_price_total = parseFloat(
              this.form.cash_price_total,
            ).toFixed(2)
          } else if (this.form.payment == 'debit_card') {
            this.form.installment_tax = this.logist[this.form.payment]
            this.form.forward_price = parseFloat(
              parseFloat(this.form.cash_price) +
                (parseFloat(this.form.cash_price) *
                  parseFloat(this.form.installment_tax)) /
                  100,
            ).toFixed(2)
            this.form.forward_price_total = parseFloat(
              parseFloat(this.form.cash_price_total) +
                (parseFloat(this.form.cash_price_total) *
                  parseFloat(this.form.installment_tax)) /
                  100,
            ).toFixed(2)
          } else if (
            this.form.payment == 'by_financial' ||
            this.form.payment == 'by_financial_down_payment'
          ) {
            const installment_tax = JSON.parse(this.logist[this.form.payment])
            const tax = parseFloat(
              installment_tax[this.form.installment_number - 1],
            )
            if (isNaN(tax)) {
              this.form.installment_tax = 0
            } else {
              const installment_cash = parseFloat(
                parseFloat(this.form.cash_price) * parseFloat(tax),
              ).toFixed(2)
              const total_price_cash =
                installment_cash * this.form.installment_number
              this.form.installment_tax = (
                (total_price_cash * 100) / this.form.cash_price -
                100
              ).toFixed(2)
            }
        ...
        const cash_price = isNaN(parseFloat(this.form.cash_price))
          ? 0
          : parseFloat(this.form.cash_price)
        const down_payment = isNaN(parseFloat(this.form.down_payment))
          ? 0
          : parseFloat(this.form.down_payment)
        const cash_price_total = isNaN(parseFloat(this.form.cash_price_total))
          ? 0
          : parseFloat(this.form.cash_price_total)
        const installment_tax = isNaN(parseFloat(this.form.installment_tax))
          ? 0
          : parseFloat(this.form.installment_tax)
        this.form.forward_price = parseFloat(
          cash_price -
            down_payment +
            ((cash_price - down_payment) * installment_tax) / 100,
        ).toFixed(2)
        this.form.forward_price_total = parseFloat(
          cash_price_total -
            down_payment +
            ((cash_price_total - down_payment) * installment_tax) / 100,
        ).toFixed(2)
      } catch (e) {
        this.form.installment_tax = 0
      }
    ...
    loadEquipments(equipments) {
      ...
      for (const i in equipments) {
        ...
        this.form.equipments[equipment.index].discount = isNaN(
          parseFloat(equipment.discount),
        )
          ? 0
          : parseFloat(equipment.discount)
        this.form.equipments[equipment.index].final_price = isNaN(
          parseFloat(equipment.final_price),
        )
          ? 0
          : parseFloat(equipment.final_price)
      }
    ...
    addEquipment() {
      const lastIndex = Object.keys(this.form.equipments)[
        Object.keys(this.form.equipments).length - 1
      ]
